...
export function TargetCursor({
  ...
  useEffect(() => {
    ...
    const initCursor = () => {
      ...
      // If container is specified but not found, wait a bit and retry
      ...
      const enterHandler = (e: MouseEvent) => {
        ...
        // If container is specified, check if mouse is inside it
        if (container) {
          const rect = container.getBoundingClientRect();
          const isInside =
            e.clientX >= rect.left &&
          ...
          // Also verify that the target or its parent is inside the container
          if (!container.contains(targetElement)) {
            return;
          }
        }
        
        const directTarget = targetElement;
      ...
      eventTarget.addEventListener("mouseover", enterHandler as EventListener, true);
      ...
      cleanupFn = () => {
        ...
        eventTarget.removeEventListener("mouseover", enterHandler as EventListener, true);
  ...
  // Also check isMobile after mount to avoid hydration issues
  if (!isMounted) {
    return null;
  }
